            ...
            while (true)
            {
                try
                {
                    ...
                    var oldStreamDataDict = allStreamData
                                            // group by type
                                            .GroupBy(entry => entry.Key.Type).ToDictionary(entry => entry.Key,
                    ...
                    var newStreamData = await oldStreamDataDict.Select(x =>
                    {
                        ...
                        return Task.FromResult<IReadOnlyCollection<StreamData>>(new List<StreamData>());
                    ...
                    foreach (var newData in newStreamData.SelectMany(x => x))
                    {
                        ...
                        if (!oldStreamDataDict.TryGetValue(key.Type, out var typeDict) || !typeDict.TryGetValue(key.Name, out var oldData) || oldData is null)
                        {
