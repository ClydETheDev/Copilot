...
public class MultiGreets : MewdekoModuleBase<MultiGreetService>
{
    ...
    public async Task MultiGreetAdd ([Remainder] ITextChannel? channel = null)
    {
        ...
        {
            case true:
                await ctx.Channel.SendConfirmAsync($"Added {channel.Mention} as a MultiGreet channel!").ConfigureAwait(false);
                break;
            case false:
        ...
        if (message is null)
        {
            ...
            switch (response)
            {
                case "preview":
                    ...
                    var replacer = new ReplacementBuilder().WithUser(ctx.User).WithClient(ctx.Client as DiscordShardedClient).WithServer(ctx.Client as DiscordShardedClient, ctx.Guild as SocketGuild).Build();
                    ...
                    {
                        await ctx.Channel.SendMessageAsync(plainText, embeds: embedData, components: components2.Build()).ConfigureAwait(false);
                    }
                    else
                    {
