...
public class SlashAfk : MewdekoSlashModuleBase<AfkService>
{
    ...
    public async Task CustomAfkMessage(string embedCode)
    {
        ...
        if (embedCode == "-")
        {
            ...
            await ctx.Interaction.SendConfirmAsync("Afk messages will now have the default look.").ConfigureAwait(false);
            return;
        }
    ...
    public async Task AfkLength(int num)
    {
        ...
        else
        {
            ...
            await ctx.Interaction.SendConfirmAsync($"AFK Length Sucessfully Set To {num} Characters").ConfigureAwait(false);
        }
    }
            ...
            case "onmessage":
                {
                    ...
                    await ctx.Interaction.SendConfirmAsync("Afk will be disabled when a user sends a message.").ConfigureAwait(false);
                }
                break;
            case "ontype":
                {
                    await Service.AfkTypeSet(ctx.Guild, 2).ConfigureAwait(false);
                    await ctx.Interaction.SendConfirmAsync("Afk messages will be disabled when a user starts typing.").ConfigureAwait(false);
                }
                break;
            case "selfdisable":
                {
    ...
    public async Task AfkTimeout(string input)
    {
        ...
        {
            await ctx.Interaction.SendErrorAsync("The maximum Afk timeout is 2 Hours. Minimum is 1 Second.").ConfigureAwait(false);
            return;
        }

        await Service.AfkTimeoutSet(ctx.Guild, Convert.ToInt32(time.Time.TotalSeconds)).ConfigureAwait(false);
        await ctx.Interaction.SendConfirmAsync($"Your AFK Timeout has been set to {time.Time.Humanize()}").ConfigureAwait(false);
    }
    ...
    public async Task AfkUndisable(ITextChannel channel)
    {
        ...
        if (mentions.Count == 0)
        {
            await ctx.Interaction.SendErrorAsync("The channels you have specifed are not set to ignore Afk!").ConfigureAwait(false);
            return;
        }
        ...
        if (!list.Except(toremove).Any())
        {
            ...
            await ctx.Interaction.SendConfirmAsync("Mewdeko will no longer ignore afk in any channel.").ConfigureAwait(false);
            return;
        }
    ...
    public async Task AfkRemove(IGuildUser user)
    {
        ...
        await ctx.Interaction.SendEphemeralConfirmAsync($"AFK Message for {user.Mention} has been disabled!").ConfigureAwait(false);
    }
}