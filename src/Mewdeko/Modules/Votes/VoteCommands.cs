...
public class Vote : MewdekoModuleBase<VoteService>
{
    ...
    public async Task VoteRoleAdd(IRole role, StoopidTime time = null)
    {
        if (time is not null)
        {
            ...
            if (!added.Item1)
            {
                await ctx.Channel.SendErrorAsync($"Adding vote role failed for the following reason: {added.Item2}");
            }
            else
            {
    ...
    public async Task VoteRoleEdit(IRole role, StoopidTime time)
    {
        ...
        if (!update.Item1)
            await ctx.Channel.SendErrorAsync($"Updating vote role time failed due to the following reason: {update.Item2}");
        else
